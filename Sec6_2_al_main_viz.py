...
from Sec6_2_plot_distribution import plot_data_2d
...
# Dictionary to store test R2 values for each strategy
R2s_dict = {}

# Instantiate all strategy classes
al_RS = RS()
...
for strategy in strategies:
    ...
    # Iterate over active learning cycles
    for cycle in tqdm(range(num_cycles), desc=f"{desc_text} ", ncols=80):

        # Initialize XGBoost model with appropriate parameters
        ...
        if strategy == "RS":
            ...
            print("An undefined strategy was encountered.")  # Alert if an undefined strategy is encountered
            selected_indices = []

        if len(current_labeled_indices) in [10, 50, 100, 150]:
            plot_data_2d(X_train_full_df, y_train_full_df, current_labeled_indices, feature_idx=0)
...
# Print or save results for all strategies
print(R2s_dict)
